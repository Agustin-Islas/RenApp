...
class FourWeeksDialysisPdfService {
  ...
  Future<Uint8List> build4WeeksReport({
    ...
    document.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          _header(patient, dateRangeLabel),
  ...
  Future<void> download(Uint8List bytes, String filename) async {
    if (!kIsWeb) {
      ...
      await Share.shareXFiles(
        [file],
        text: 'Registro 4 Semanas de diálisis peritoneal',
        subject: filename,
      );
      return;
    }
  ...
  pw.Widget _header(MeResponse patient, String dateRangeLabel) {
  ...
  pw.Widget _summary(FourWeeksUltrafiltrationSummary summary) {
